...
func (s *BookService) ReturnBook(payload *input.ReturnBookDTO) error {
	...
	if firstBookModelRes.BorrowStock >= firstBookModelRes.Stock {
		return status.Error(codes.FailedPrecondition, "all book have already been returned")
	}

	firstBorrowRecordModelRes, err := s.BorrowRecordDatabaseSQLRepository.FirstBorrowRecord(&types.QuerySQL{
		Searches: [][]types.SearchQuerySQLOperation{
			{
				{Field: "user_id", Operator: "=", Value: payload.UserID},
				{Field: "book_id", Operator: "=", Value: firstBookModelRes.ID},
				{Field: "return_date", Operator: "IS NULL"},
			},
		},
		Clauses: []types.ClauseExpression{
			clause.Locking{Strength: "UPDATE"},
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return status.Error(codes.NotFound, "borrow record not found")
		}
		return err
	}

	borrowRecordSave := firstBorrowRecordModelRes

	returnDate := time.Now()
	borrowRecordSave.ReturnDate = &returnDate
